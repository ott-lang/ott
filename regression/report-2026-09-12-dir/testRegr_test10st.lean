/- generated by Ott 0.34 from: /home/pes20/claude-home/ott/regression/../tests/test10st.ott -/




set_option linter.unusedVariables false

abbrev termvar := String /- term variable -/
abbrev typvar := String /- type variable -/

inductive Typ where /- type -/
    | T_var : (X:typvar) -> Typ /- variable -/
    | T_arrow : (T:Typ) -> (T':Typ) -> Typ /- function -/

  deriving Inhabited

inductive Term where /- term -/
    | t_Var : (x:termvar) -> Term /- variable -/
    | t_Lam : (x:termvar) -> (t:Term) -> Term /- abstraction -/
    | t_App : (t:Term) -> (t':Term) -> Term /- application -/

  deriving Inhabited

abbrev G := List (termvar×Typ)

open Term Typ
def bound (x:termvar) (t0:Typ) (g:G) :=
  match g with
  | (x',t')::g' => if x=x' then t0=t' else bound x t0 g'
  | [] => false


/- - subrules - -/
def is_v_of_t (t5:Term) : Bool :=
  match t5 with
  | (t_Var x) => false
  | (t_Lam x t) => (true)
  | (t_App t t') => false


/- - library functions - -/
def list_minus [BEq a] (l1:List a) (l2:List a) : (List a) :=
  match l1 with
  | [] => []
  | h::t => if List.elem h l2 then list_minus t l2 else h::(list_minus t l2)


/- - free variables - -/
def fv_t (t5:Term) :  List termvar :=
  match t5 with
  | (t_Var x) => [x]
  | (t_Lam x t) => (list_minus (fv_t t) [x])
  | (t_App t t') => (fv_t t) ++ (fv_t t')



/- - substitutions - -/
def tsubst_t (t5:Term) (x5:termvar) (t_6:Term) : Term :=
  match t_6 with
  | (t_Var x) => (if x=x5 then t5 else (t_Var x))
  | (t_Lam x t) => t_Lam x (if  List.elem x5 ([x]) then t else (tsubst_t t5 x5 t))
  | (t_App t t') => t_App (tsubst_t t5 x5 t) (tsubst_t t5 x5 t')


/- definitions -/


/- defns Jtype -/
inductive GtT : G -> Term -> Typ -> Prop where    /- defn GtT -/
| GtT_value_name: forall (G:_root_.G) (x:_root_.termvar) (T:_root_.Typ),
( (bound  x   T   G ) )
 -> 
GtT G (t_Var x) T

| GtT_apply: forall (G:_root_.G) (t:_root_.Term) (t':_root_.Term) (T2:_root_.Typ) (T1:_root_.Typ),
(GtT G t (T_arrow T1 T2)) ->
(GtT G t' T1)
 -> 
GtT G (t_App t t') T2

| GtT_lambda: forall (G:_root_.G) (x1:_root_.termvar) (t:_root_.Term) (T1:_root_.Typ) (T:_root_.Typ),
(GtT  (( x1 , T1 ):: G )  t T)
 -> 
GtT G (t_Lam x1 t) (T_arrow T1 T)

/- definitions -/


/- defns Jop -/
inductive reduce : Term -> Term -> Prop where    /- defn reduce -/
| ax_app: forall (x:_root_.termvar) (t12:_root_.Term) (v2:_root_.Term),
(is_v_of_t v2)
 -> 
reduce (t_App  (t_Lam x t12)  v2)  ( tsubst_t  v2   x   t12  ) 

| ctx_app_fun: forall (t1:_root_.Term) (t:_root_.Term) (t1':_root_.Term),
(reduce t1 t1')
 -> 
reduce (t_App t1 t) (t_App t1' t)

| ctx_app_arg: forall (v:_root_.Term) (t1:_root_.Term) (t1':_root_.Term),
(is_v_of_t v) ->
(reduce t1 t1')
 -> 
reduce (t_App v t1) (t_App v t1')



