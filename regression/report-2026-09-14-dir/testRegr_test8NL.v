...
Inductive typscheme : Set := 
 | TS_ts (_:list typvar) (typexpr5:typexpr).
...
(** subrules *)
Definition is_value_of_expr (e5:expr) : bool :=
  match e5 with
...
Definition tsubst_typscheme (sub:list (typvar*typexpr)) (ts5:typscheme) : typscheme :=
  match ts5 with
  | (TS_ts typvar_list typexpr5) => TS_ts typvar_list (tsubst_typexpr (list_minus2 eq_typvar sub typvar_list) typexpr5)
end.
...
Definition ftv_typscheme (ts5:typscheme) : list typvar :=
  match ts5 with
  | (TS_ts typvar_list typexpr5) => (app nil (list_minus eq_typvar (ftv_typexpr typexpr5) typvar_list))
end.
...
with Get : G -> expr -> typexpr -> Prop :=    (* defn Get *)
 ...
 | Get_lambda : forall (G5:G) (x1:value_name) (e:expr) (t1 t:typexpr),
     Get (G_vn G5 x1 (TS_ts nil t1)) e t ->
